...
<!-- 数据表工具栏，使用bootstrap -->
	<!-- content begin background:#B0B0B0-->
			<div class="container" style="margin-top: 20px;box-shadow: 3px 3px 10px;width:100%;margin-left:30px">
					<div class="row" >
							<table  class="table table-hover table-bordered" id="table" style="width:100%" >
							<tr>
									<td>单号：</td>
									<td colspan="6"><span id="dId" style="color:red"><span id="orderId">DB2017111701</span></span></td>
									<td>           
									</td>
							</tr>
								<tr>
					...
					<input id="combo1" class="mini-combobox" name="from"  style="width:150px;" textField="wname" valueField="wid" emptyText="请选择仓库"
					    url="${pageContext.request.contextPath }/warehouse/getWarehouse.action"  required="true" allowInput="true" showNullItem="true" nullItemText="请选择..."/>   </td>	
									<td>调入仓库：</td>	
									<td colspan="2">    
								<input id="combo2" class="mini-combobox" name="to" style="width:150px;" textField="wname" valueField="wid" emptyText="请选择仓库"
								    url="${pageContext.request.contextPath }/warehouse/getWarehouse.action"   required="true" allowInput="true" showNullItem="true" nullItemText="请选择..."/>   
    							</td>	
									<td>经办人：</td>	
   ...
   <!--
   			...
   			3.修改完成后提交，批量增加
     -->
     <div class="container" style="margin-top: 10px;width:100%;margin-left:16px">
    <div style="width:100%;">
            ...
            <table style="width:100%;">
                <tr>
                    <td style="width:100%;">
                        ...
                        <span class="separator"></span>
                        <a class="mini-button" iconCls="icon-save" onclick="doAdd()" plain="true">提交调度单</a>          
                    </td>
    ...
    <div id="datagrid1" class="mini-datagrid" style="width:100%;height:280px;margin:0" 
     ...
     <script type="text/javascript">
       ...
       var str;
         window.onload = function generateId(){
    	  //据时间自动生成单号
    	    var time2 = new Date().Format("DByyyyMMddhhmmss");
    	  	str = time2+"";
       ...
       function doAdd(){
    	  ...
    	  var date =new Date(); 
    	   var from =mini.get("combo1").getSelected().wid;
    	   var to =mini.get("combo2").getSelected().wid;
    	   var sfid =mini.get("combo3").getSelected().sfid;
    	   if(from===to){
    		   //mini的提示框,校验仓库
    		   mini.alert("调出调入仓库不能相同！");
    		   return;
    	   }
    	 	//获取单号
    	 	var dispid = str;
    	    ...
    	    if(confirm("确认提交?")){
    		   //怎么获取表格中的值，先手动选中za============
    			    $.ajax({
                url: "${pageContext.request.contextPath }/warehouse/addDispatchers.action",
                dataType:"json",//预期的服务器响应的数据类型
                contentType: "application/json; charset=utf-8",//发送数据到服务器时所使用的内容类型
                data:json,
                type: "post",
