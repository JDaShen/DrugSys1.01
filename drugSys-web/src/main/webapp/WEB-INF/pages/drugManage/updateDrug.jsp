...
<body>    
        ...
        <div style="padding-left:11px;padding-bottom:5px;">
            <table style="table-layout:fixed;">
                <tr>
                    <td >药品编号：</td>
                    <td >    
                        <input name="diid" class="mini-textbox" readonly="readonly" required="true"  emptyText="请输入编号"/>
                    </td>
                    <td >所属分类：</td>
                    <td style="height: 50px;">    
                        ...
                        <input id="drugDetail" name="ddid" class="mini-combobox" valueField="ddid" textField="ddname" 
                             ...
                             emptyText="请选择所属分类"
                            />
                         <input type="hidden" id="ddidx" name="ddidx" >
                    </td>
                </tr>
                <tr>
                    <td >药品名称：</td>
    ...
    <script type="text/javascript">
		...
		var alex;//ddid,数字型的字符串
		var ux; //二级类名
        function UpdateData() {
            if (mini.getbyName("ddid").getValue() === ux) {
            	 mini.getbyName("ddid").setValue(alex);
			}
            var o = form.getData(); 
            var json = mini.encode(o);   //序列化成JSON
            $.ajax({
                url: "${pageContext.request.contextPath }/drugMC/updateDrug.action",
                type: "post",
                data: json,
        ...
        function SetData(data) {
            if (data.action == "edit") {
                ...
                $.ajax({
                    url: "${pageContext.request.contextPath }/drugMC/getDrugById.action?diid=" + data.diid,
                    cache: false,
                    success: function (text) {
                        ...
                        ux = o.ddname;
                        alex = o.ddid;
                        mini.getbyName("dtid").setValue(o.dtname);
                        mini.getbyName("ddid").setValue(o.ddname);
